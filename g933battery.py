...
def get_battery_state():
        ...
        for intf in cfg:
            if dev.is_kernel_driver_active(intf.bInterfaceNumber):
                try:
                    ...
                    sys.exit("Could not detach kernel driver from "
                             "interface({0}):{1}".format(intf.bInterfaceNumber,
                                                         str(e)))
    ...
    while True:
        try:
            ...
            if len(data) > 6 and not 135 == data[6] and not 145 == data[6]:
                state = data[6]
                battery_status = 0
                # Current best effort trying to extract battery status
                if state != 0:
                    battery_status = get_battery_level(data[4], data[5])
                print("Battery:~{}% (estimated from:{}/2,{}/255)".format(
                    battery_status, data[4] - 13, data[5]), "Status:",
                    status(state))
                break
